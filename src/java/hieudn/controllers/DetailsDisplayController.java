...
public class DetailsDisplayController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            int currentPage;
            String search = request.getParameter("hotel");
            Hotel h = new Hotel(search);
            if (request.getParameter("pageIDPaging1") == null) {
                currentPage = 1;
            } else {
                currentPage = Integer.parseInt(request.getParameter("pageIDPaging1"));
            }
            int pageMaxSize = 5;
            BookingBLO blo = new BookingBLO();
            List<Rooms> result = blo.getRoomByHotelId(h, currentPage, pageMaxSize);
            if (result != null) {
                request.setAttribute("ROOM", result);
                request.setAttribute("CURRENT_PAGE", currentPage);
                request.setAttribute("ROOM_COUNT", blo.getAmountRoom(search, pageMaxSize));
                url = SUCCESS;
            }
